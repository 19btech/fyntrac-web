...
const AddSubledgerMappingDialog = ({ open, onClose, editData }) => {
  ...
  const handleAddSubledgerMapping = async () => {
    ...
    try {
      ...
      // Rule 2: Duplicate transaction + sign + accountSubType
      const isDuplicate = sameTransaction.some(
        r => r.sign?.toLowerCase() === signLower && r.accountSubType?.toLowerCase() === subTypeLower
      );
      if (isDuplicate) {
        setErrorMessage('Duplicate posting rule found.');
        setShowErrorMessage(true);
        return;
      }

      // Rule 3: Same transaction + sign but conflicting entryType
      const sameSign = sameTransaction.filter(r => r.sign?.toLowerCase() === signLower);
      const conflictingEntryType = sameSign.some(r => r.entryType?.toLowerCase() !== entryTypeLower);
      if (conflictingEntryType) {
        setErrorMessage('Multiple Entry Types found for the same transaction name and Sign.');
        setShowErrorMessage(true);
        return;
      }

      // Rule 1: Adding a sign for a transaction that already has entries under a DIFFERENT sign (add mode only)
      if (!id) {
        const hasDifferentSign = sameTransaction.some(r => r.sign?.toLowerCase() !== signLower);
        if (hasDifferentSign) {
          setErrorMessage('Only one posting rule is allowed per transaction and sign. Combination with opposite sign will be generated by the system.');
          setShowErrorMessage(true);
          return;
        }
      }
