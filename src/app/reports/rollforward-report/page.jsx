...
const RollforwardReportPage = () => {
  ...
  const [reportSource, setReportSource] = useState({ value: 'instrument-rollforward', label: 'Instrument Level' });
  ...
  const downloadReport = () => {
    ...
    reportingApi.post(`/${reportSource.value}/download`, filteredCriteriaList, {
      ...
      .then(response => {
        ...
        let fileName = 'rollforward-report.xlsx';
  ...
  return (
    <div>
      <Grid container spacing={3}>
        <Grid size="auto">
          <div className="left">
            <GridHeader>Rollforward Report</GridHeader>
          </div>
        </Grid>
        <Grid size={6} />
        <Grid size="grow">
            ...
            <Stack direction="row" spacing={1}>
              <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'center' }}>
                {reportSources.map((source) => {
                  const selected = reportSource?.value === source.value;
                  return (
                    <Chip
                      key={source.value}
                      label={source.label}
                      onClick={() => updateReportSource(source)}
                      size="small"
                      sx={{
                        fontWeight: selected ? 700 : 500,
                        fontSize: '0.78rem',
                        bgcolor: selected ? 'rgba(59,130,246,0.15)' : 'rgba(219,234,254,0.5)',
                        color: selected ? '#1d4ed8' : '#3b82f6',
                        border: selected ? '1.5px solid rgba(59,130,246,0.5)' : '1px solid rgba(147,197,253,0.6)',
...
export default RollforwardReportPage;